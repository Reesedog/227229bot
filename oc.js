...
(function() {
        ...
        if (isDragging) {
            if (!mouse_throttle_flag) {
                ...
                mouse_throttle = setTimeout(() => {
                    ...
                    floatWindow.style.right = (window.innerWidth - mouseMoveX + mouseDownX - initX - 70) + "px";
    ...
    function addButtonToContainer() {
        ...
        button.onclick = function() {
            let textarea = document.querySelector('textarea.ChatSend-txt');
            if (textarea) {
                let textContent = hoveredElement.querySelector('.text-edf4e7').textContent;
                textarea.value = textContent;

                // 假设存在一个具有 'ChatSend-btn' 类的发送按钮（您可能需要调整这一点）
                let sendButton = document.getElementsByClassName('ChatSend-button')[0].click();
                if (sendButton) {
                    sendButton.click(); // 点击发送按钮以提交聊天消息
                }
            }
        };
    }
    ...
    button.textContent = '关闭复读';
    document.body.appendChild(button);

    // 为按钮添加点击事件监听器
    button.addEventListener('click', () => {
        blockDanmu = !blockDanmu;
        button.textContent = blockDanmu ? '开启复读' : '关闭复读';
    });
